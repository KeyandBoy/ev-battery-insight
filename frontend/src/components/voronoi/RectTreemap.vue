...
function render() {
  ...
  nodes.append('rect')
    ...
    .attr('fill', d => {
      ...
      if (!d.children) {
        const t = d.depth / (root.height || 1) * 0.45
        return d3.interpolateRgb(base, '#f0f5ff')(t)
      }
      return d3.interpolateRgb(base, '#ffffff')(0.85)
    })
    .attr('stroke', d => d.depth === 0 ? 'none' : 'rgba(255,255,255,0.6)')
  ...
  nodes.filter(d => {
    ...
    return d.depth > 0 && rw > 30 && rh > 14
  }).append('text')
    .attr('x', 4).attr('y', d => d.children ? 13 : (d.y1 - d.y0) / 2 + 4)
    .attr('font-size', d => d.children ? 10 : Math.min(12, (d.x1 - d.x0) / 6))
    .attr('fill', d => d.children ? '#555' : '#333')
